...
public class MultiCriteriaManager extends StructuralGoalManager implements Serializable {
    ...
    public MultiCriteriaManager(List<TestFitnessFunction> targets) {
        ...
        for (Criterion criterion : Properties.CRITERION) {
            switch (criterion) {
                ...
                case CBRANCH:
                    ...
                    break;
                case METHODPAIR:
                    // MethodPair goals have no branch sub-dependencies; DynaMOSA
                    // will treat them as root goals directly.
                    break;
                default:
        ...
        // initialize current goals: only add root branches when branch-based criteria are active
        boolean hasBranchCriteria = ArrayUtil.contains(Properties.CRITERION, Criterion.BRANCH)
                || ArrayUtil.contains(Properties.CRITERION, Criterion.CBRANCH)
                || ArrayUtil.contains(Properties.CRITERION, Criterion.IBRANCH)
                || ArrayUtil.contains(Properties.CRITERION, Criterion.ONLYBRANCH);
        if (hasBranchCriteria) {
            this.currentGoals.addAll(graph.getRootBranches());
        }
        // For non-branch criteria (e.g. METHODPAIR), seed current goals with the
        // uncovered fitness functions directly so DynaMOSA has something to optimise.
